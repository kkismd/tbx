...
今後は、ゲームルールや実装判断の正本を `docs/specs/galactic_exodus/` に集約し、実装 issue は対応する spec file を `Source of truth` として参照する。
...
| `docs/specs/galactic_exodus/` | 仕様正本。実装 issue が参照する source of truth。 |
| `docs/design/` | 表示案・比較サンプル・設計メモ。ゲームルール正本としては扱わない。 |
| fixtures / snapshots | 実装済み挙動の regression 固定。正本仕様と矛盾した場合は正本仕様を優先する。 |
| README | 実行方法・ユーザー向け説明。詳細仕様は docs/specs へリンクする。 |
...
```text
Source of truth:
  docs/specs/galactic_exodus/<spec-file>.md
