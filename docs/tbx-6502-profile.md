...
初期 `generic-6502` プロファイルは、fig-FORTH に着想を得た **間接スレッディング方式（indirect threading）** を採用する。16bit の実行トークンは、ワードのコードフィールドまたはそれに相当する辞書エントリを間接参照する。

M2/M3 では、実行トークンの具体的な辞書レイアウト、コードフィールドの表現、dispatcher の内部構造を決めてよい。ただし、direct threading へ変更してはならず、次の性質を守る。
...
静的配列の lowering・bounds check と、seed・bounded `RND` は初期ターゲット全体では必須だが、M2 の最小 VM 契約には含めない。これらは後続マイルストーンで追加し、遅くとも M6 の検証までに実装する。
...
- 手書きの間接スレッディングプログラムだけで call、branch、memory、output を検証できる。
