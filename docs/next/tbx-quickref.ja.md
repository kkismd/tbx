...
重要な点として、これは**実行時 arity の宣言ではなく、戻るときに引数を自動消費する仕組みでもない**。必要な値の消費はワード本体のスタック操作として記述する。

例として、1 個の引数を消費して判定結果だけを残すワードは次のように書ける。
...
`RND(N)` は `1..N` の一様な整数を返す。`N` は正でなければならない。
...
設計理由が必要な場合は、コード中で参照されている ADR issue と Milestone #17 `TBX-Next` を確認すること。
