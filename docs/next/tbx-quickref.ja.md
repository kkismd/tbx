...
- 定義ワード (`DEF ... END`) 内では `I,J,K,L,M,N,X,Y` を invocation-local スクラッチ として宣言なしで使える
...
スクラッチ名はグローバルの名前表へ公開されない。そのため構文位置が明確にワード呼出しや配列参照を要求する場合はスクラッチが横取りしない。たとえば、その名前のwordまたはarray bindingが存在するなら、文の先頭の `I` と `I()` はワード呼出し、`@I[index]` は配列参照として扱われる。

`DEF` headerの局所参照名にはスクラッチ名を使えない。次はエラーになる。
...
代入は `LET name = expression` で行う。トップレベルのtargetは宣言済みグローバルでなければならない。定義ワード内では、固定スクラッチ名への `LET` は現在の呼出しのスクラッチへ書き込む。
...
局所参照名は同じdefinition内のスクラッチと名前を共有できない。
`I,J,K,L,M,N,X,Y` をheaderの局所参照名に指定すると定義時にエラーになる。
...
- グローバル配列のstorage: `crates/tbx-next/src/グローバル_array.rs`
