...
- `VAR`, `LET`, `POP_TO`, `EVAL`, `DEF`, `IF`, `SYNTAX`, `USE`, `PRINT` はソース処理ワードであり、通常のランタイムワードとは役割が異なる
...
`POP_TO name` は、データスタック最上位の1値を指定したスカラーへ格納し、その値をスタックから消費するソース処理ワードである。

```text
(x --)
```

`LET name = expression` が右辺式を評価してから格納するのに対し、`POP_TO name` は**すでにデータスタック上にある値**を格納する。
...
書き込み先の解決規則はスカラーの `LET` と同じである。

- トップレベルでは、宣言済みグローバル変数へ書き込む
- 定義ワード内で `I,J,K,L,M,N,X,Y` を指定した場合は、その呼出しの一時変数へ書き込む
...
局所参照名は読み取り専用であり、`LET` / `POP_TO` の書き込み先には指定できない。
