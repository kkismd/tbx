...
保存領域ではない。そのうえで、短命な計算途中値、ループのインデックス、座標、乱数値など、
...
同じ意味のまま増減する各代入へ同じコメントを繰り返す必要はない。
...
特にループのインデックスでも例外ではない。次のようにループ本体からランタイムワードを
呼び出し、その後の `NEXT` や継続判定でもインデックス値が必要な場合、その値が
呼出し先から保存されることを前提にしてはならない。
...
避けるという暗黙の呼出し規約は置かない。呼出し先もA-Zを通常のグローバルスカラー
として使用できる。
