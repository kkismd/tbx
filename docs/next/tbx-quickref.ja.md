...
`A` から `Z` は通常のグローバルスカラーであり、スクラッチ専用の別種の
storageではない。そのうえで、短命な計算途中値、ループindex、座標、乱数rollなど、
単一の処理内で完結する作業値には1文字変数を使うことを推奨する。

**原則として、1文字変数の値をランタイムワード呼出しの前後で継続利用しない。**

1文字変数へ新しい意味を割り当てる最初の代入付近では、値の意味をコメントに残す。
...
LET R = RND(100) # Klingon generation roll
IF R <= 80
  ...
ENDIF
```

同じ意味のままincrement/decrementする各代入へ同じコメントを繰り返す必要はない。
`I` / `J` / `K` をindex、`R` を乱数roll、`X` / `Y` を座標に使うことは
読みやすい慣習になり得るが、各文字の用途は言語仕様として固定されていない。

ランタイムワード呼出しをまたいで必要な値、ゲーム全体の状態、複数箇所から意味を
持って参照する値には、意味を表す名前付き `VAR` を使う。

特にループindexでも例外ではない。次のようにループ本体からランタイムワードを
呼び出し、その後の `NEXT` や継続判定でもindex値が必要な場合、`I` が
calleeから保存されることを前提にしてはならない。
...
callerが `I` / `J` / `K` を使用中であることを理由に、calleeが同じ1文字変数を
避けるという暗黙の呼出し規約は置かない。calleeもA-Zを通常のグローバルスカラー
として使用できる。

ワードへの入力と返値にはデータスタックを使える。1文字変数は、ワード間の値渡しを
置き換える仕組みではなく、呼出しをまたがない短命な作業値を簡潔に書くための
コード規律として使う。
