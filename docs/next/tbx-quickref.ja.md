...
- compiled word (`DEF ... END`) 内では `I,J,K,L,M,N,X,Y` を invocation-local scratch として宣言なしで使える
...
現在の `VAR` はsource-processing session全体へグローバルな名前を公開する。宣言された値はワード呼出しをまたいで保持される。現行 TBX のローカル `VAR` と同じものとして扱わないこと。
...
- 各compiled word呼出しごとに8個の独立したslotを持つ
- 呼出し開始時はすべて `0`
- 呼出し元と呼出し先は別のscratchを持つため、calleeが同じ文字を使ってもcallerの値を変更しない
- 通常の `END` 到達または `RETURN` でその呼出しが終わると、そのscratchの寿命も終わる
- top-levelにはscratchはなく、未宣言の `I` などは通常の未定義名エラーになる
- 各文字の用途は言語仕様として固定されていない

`VAR I` のような同名globalを宣言すること自体は許される。ただしcompiled word内の**裸の値参照 `I`** と **`LET I = ...`** は、そのglobalではなく現在の呼出しのscratchへ解決される。top-levelの `I` は宣言済みglobalへ解決される。

scratch名はglobal binding tableへ公開されない。そのため構文位置が明確にword callやarray参照を要求する場合はscratchが横取りしない。たとえば、その名前のwordまたはarray bindingが存在するなら、statement先頭の `I` と `I()` はword call、`@I[index]` はarray参照として扱われる。

`DEF` headerの局所参照名にはscratch名を使えない。次はエラーになる。
...
8個で足りない場合は、まず値の再利用・再計算・小さなdata stack退避・処理の分割を検討する。scratch不足だけを理由に短命値をglobalへ移すことは避ける。長寿命または呼出しをまたいで共有すべき状態には、意味を表す名前の `VAR` を使う。

### LET

代入は `LET name = expression` で行う。top-levelのtargetは宣言済みglobalでなければならない。compiled word内では、固定scratch名への `LET` は現在の呼出しのscratchへ書き込む。
...
局所参照名は同じdefinition内のscratchと名前を共有できない。
`I,J,K,L,M,N,X,Y` をheaderの局所参照名に指定するとdefinition時にエラーになる。
...
| 値 | 現在は `i16` 整数のみ |
| 真偽 | `0` が偽、0 以外が真 |
| 変数 | persistent stateは `VAR` で宣言するグローバル変数。compiled word内では `I..N,X,Y` が呼出しごとのローカルスクラッチ |
