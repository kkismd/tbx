...
作業は**必ず git worktree を使って行う**。`create-pull-request` スキル（`.github/skills/create-pull-request/SKILL.md`）を**実際に読んでから**手順に従う。

worktreeの作成手順:
```bash
# メインリポジトリ（tbx/）で実行する
git pull
git worktree add ../tbx-issue-N-short-description -b issue/N-short-description
cd ../tbx-issue-N-short-description
```

**重要な注意点**:
- mainブランチへの直接コミットは禁止。必ずworktreeでブランチを切る
- **blueprint.mdの編集はworktree内で行う**（メインリポジトリ内で直接編集しない）
