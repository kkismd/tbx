...
0. **ループ先頭での上限チェック**：ループカウンターを確認し、上限に達していれば修正を行わず最終レビューフェーズへ進む。
   ```sql
   SELECT CAST(value AS INTEGER) AS loop_count FROM session_state WHERE key = 'review_loop_count';
   ```
   `loop_count >= 3` の場合: 修正は行わず、**ステップ6最終レビューフェーズへ進む**（最終レビューは専用フェーズで常に1回実施するため、ここでは実施しない）
...
4. **新しいコメントもレビューも追加されていない**（どちらの件数も変化なし）→ 指摘なし。ループを終了して**ステップ6最終レビューフェーズへ進む**。

5. **新しいコメントまたはレビューが追加された場合**、追加された内容を確認する：
   - **🔴/🟡/🟢 のいずれも含まれない**（Approveレビューのみ）→ ループを終了して**ステップ6最終レビューフェーズへ進む**。
   - **🟢 Info のみ含まれる**（🔴/🟡 はない）→ ループを終了して**ステップ6最終レビューフェーズへ進む**。
...
修正サイクル完了後、コードの最終状態を確認するために必ず1回レビューを実施する。

1. 現在の件数を SQL に保存する：
   ```sql
   INSERT OR REPLACE INTO session_state (key, value) VALUES ('final_review_before_comment_count', '<N_comments>');
   INSERT OR REPLACE INTO session_state (key, value) VALUES ('final_review_before_review_count', '<N_reviews>');
   ```

2. `review-implementation` エージェントを起動する：
   ```
   review-implementation エージェントを起動: PR #<PR番号> をレビューしてください
   ```

3. レビュー完了後、新しいコメント・レビューを確認する。

4. 🔴/🟡 が含まれる場合、未解消指摘として PR にコメントを追加する：
   ```bash
   cat > "$(git rev-parse --git-dir)/UNRESOLVED_COMMENT.md" << 'EOF'
   ## ⚠️ 最終レビューで指摘が検出されました

   修正サイクル後の最終レビューで以下の指摘が見つかりました。
   手動での対応をお願いします。
...
- 最終レビューの結果（以下のいずれか）：
  ...
  - 🔴/🟡 が最終レビューで検出され、PRにコメントを記録した（未解消指摘の要約）
