...
description: TBXプロジェクトのissueを読み込み、Rustコードを実装してPull Requestを作成し、修正サイクル（最大3回）と最終レビュー（常に1回）の2フェーズで品質を担保するエージェント。実装方針に不明点がある場合はissueコメントを投稿して停止し、ユーザーの返答後に再起動する確認フローを持つ。「issue #N を実装して」というプロンプトで起動する。
---
...
**再起動フロー（ステップ1でユーザーの回答を取得済み）の場合**：回答内容に従って実装方針を確定し、ステップ4へ進む。ただし、ユーザーの回答で解消しない不明点や、回答を受けて新たに生じた不明点がある場合は、通常フローと同様に確認コメントを投稿して停止する。
