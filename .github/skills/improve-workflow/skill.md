...
git worktree add ../.tbx-worktrees/improve-short-description -b improve/short-description
cd ../.tbx-worktrees/improve-short-description
```
...
git worktree remove ../.tbx-worktrees/improve-short-description
git checkout main && git pull
```

> **untracked ファイルが残っている場合**: `Cargo.lock` や `target/` など gitignore 対象のファイルが残っていると `git worktree remove` が失敗する。その場合は `--force` を付ける。
> ```bash
> git worktree remove --force ../.tbx-worktrees/improve-short-description
> ```
