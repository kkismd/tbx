...
## 必須運用ルール
- **人間レビュー必須**: すべての変更はPRとして生成し、人間がレビューするまで本番ブランチにマージしない。
...
- PRレビューまたは修正確認を始める前に [`docs/pr-review-result.md`](docs/pr-review-result.md) を必ず確認する。レビュー終了時は、同文書で定義する機械可読レビュー結果markerを最終PRコメントへ含める。
