...
Headers use a linked-list (`prev: Option<usize>`) for shadowing and lookup order. Within a session, `headers` and `dictionary` entries grow monotonically, except for the internal rollback that undoes a partially-compiled definition when `DEF ... END` fails. The array pool contains both long-lived entries (top-level allocations) and frame-local entries; frame-local arrays created during a call are freed on `EXIT` / `RETURN_VAL` using the length saved in `ReturnFrame::Call` (`saved_array_pool_len`). Strings are represented as `Cell::Str(Rc<str>)` — reference-counted immutable handles — and need no pool-based lifetime management; a `Cell::Str` may be safely shared across the data stack, variable slots, and array elements. A full reset is achieved by re-creating the VM or by recompacting from source. String literals are compiled as `Cell::Str(Rc<str>)` directly into the dictionary.
