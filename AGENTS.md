...
Headers use a linked-list (`prev: Option<usize>`) for shadowing and lookup order. Within a session, `headers` and `dictionary` entries grow monotonically, except for the internal rollback that undoes a partially-compiled definition when `DEF ... END` fails. The runtime string pool (`VM::strings`) and the array pool contain both long-lived entries (compile-time literals and top-level allocations) and frame-local entries; frame-local entries created during a call are truncated on `EXIT` / `RETURN_VAL` using the lengths saved in `ReturnFrame::Call` (`saved_string_pool_len` / `saved_array_pool_len`). A full reset is achieved by re-creating the VM or by recompacting from source. String literals are compiled as `Cell::Str` into `VM::strings`.
