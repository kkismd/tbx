...
> **注意**: JUMP_ALWAYS プレースホルダーアドレス（B）の一時保存にグローバル変数 `_ELSE_TEMP` を使用している。IMMEDIATE ワードでは `VAR` ローカル変数も使用可能になったが、ELSE は `IF...ELSE...IF...ELSE...ENDIF` のようにネストされる可能性があり、同一 IMMEDIATE ワードの再帰呼び出し（コンパイル中のネスト）があり得る。ネスト時の競合を避けるためグローバルを使用するか、コンパイルスタックを活用する実装を別途検討すること（「IMMEDIATE ワードにおける VAR の制約」セクションは削除済み）。
...
> **注意**: `_ELSE_TEMP` 同様にグローバル変数を使用している。ELSE と同じネスト競合の問題があるため、実装時に検討が必要（ELSE の注意を参照）。
