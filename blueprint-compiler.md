...
コンパイルワードの実装に使用するプリミティブ群。このうち `CS_PUSH` / `CS_POP` / `PATCH_ADDR` / `COMPILE_EXPR` は **コンパイルモード（`is_compiling = true`）専用** であり、実行モード中に呼ばれた場合はエラーとする。`APPEND` / `HERE` / `JUMP_FALSE` / `JUMP_TRUE` / `JUMP_ALWAYS` は汎用プリミティブであり、コンパイルモード以外でも使用できる。
...
[loop_start] → condition code
               JUMP_FALSE [exit_addr]
               loop body
               JUMP_ALWAYS [loop_start]
[exit_addr]  → (next instruction)
```
...
> **注意**: JUMP_ALWAYS プレースホルダーアドレス（B）の一時保存にグローバル変数 `_ELSE_TEMP` を使用している。IMMEDIATE ワードでは `VAR` ローカル変数も使用可能になったが、ELSE は `IF ... ELSE ... IF ... ELSE ... ENDIF ... ENDIF` のようにコードが複数の IF/ELSE を含む場合、同じ ELSE ワードが複数回呼び出されることがある。各呼び出しは独立したローカルフレームを持つため VAR でも安全だが、グローバル変数方式でも各呼び出し間で `_ELSE_TEMP` は競合しない。実装方針は ELSE/ELIF 実装 issue で決定すること。
...
DEF ELSE
  ...
  SET &_ELSE_TEMP, HERE        REM save JUMP_ALWAYS placeholder address (B) in global
  APPEND 0                     REM emit placeholder; DP now = else-body start
  PATCH_ADDR CS_POP            REM patch IF's BIF placeholder (A) with current DP
  CS_PUSH _ELSE_TEMP           REM push B (DictAddr) onto CS for ENDIF
END
IMMEDIATE ELSE
```
...
> **注意**: `_ELSE_TEMP` 同様にグローバル変数を使用している。ELSE と同じく、コードが複数の ELIF を含む場合に同じ ELIF ワードが複数回呼び出されることがあるが、各呼び出し間で `_ELSE_TEMP` は競合しない。実装方針は ELSE/ELIF 実装 issue で決定すること（ELSE の注意を参照）。

```
REM _ELSE_TEMP is shared with ELSE (declared once in lib/basic.tbx)
...
DEF ELIF
  ...
  CS_PUSH _ELSE_TEMP           REM push B onto CS (deeper entry)
