...
> **注意**: IMMEDIATE ワードは CALL フレームなしで呼び出されるため `VAR` ローカル変数を持てない。JUMP_ALWAYS プレースホルダーアドレス（B）の一時保存にはグローバル変数を使用する（「IMMEDIATE ワードにおける VAR の制約」参照）。
...
> **注意**: IMMEDIATE ワードは CALL フレームなしで呼び出されるため `VAR` ローカル変数を持てない。`_ELSE_TEMP` 同様にグローバル変数を使用する（「IMMEDIATE ワードにおける VAR の制約」参照）。
