...
スタックと可変配列の相互変換を用意するのは、TBX が「まず値はスタック上を流れる」というモデルを持つからである。

`FROM_ARRAY` primitive は内部実装として存在するが、利用箇所は issue #693 で整理済み。単一値取得は TUPLE projection（`T[i]`）で行うのが現行方針。多値展開が必要なケースはワード多値返却セマンティクスとして別 issue で扱う。
