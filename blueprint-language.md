...
真偽値は組み込み定数 `TRUE` / `FALSE` として表面言語から参照できる。
比較演算と論理演算も同じ `Bool` 値を返す。論理否定は primitive `NOT(V)` または
式中の単項演算子 `!expr` で表す。
...
- `&&` / `||` は short-circuit 評価し、結果を `Bool` に正規化する
- `&` / `|` は bitwise 演算子であり、左右を eager 評価する
