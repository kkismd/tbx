...
>
> 新規に生成される文字列値は、文字列リテラルを含めて `Cell::Str` として `VM::strings` に格納する。`Cell::StringDesc` / `VM::string_pool` は読み取り経路のみ残る互換表現であり、新規生成経路は存在しない。`StringDesc` 系の完全削除は #539 Phase 3 で行う。静的・動的の区別はランタイム表現ではなく寿命管理の概念として整理を続ける。
...
#### word 内からグローバル変数へ文字列リテラルを直接代入することは現状できない

`DEF ... END` 本体中に書かれた文字列リテラル（例: `SET &G, "inside"`）は、コンパイル時に `VM::strings` へ push されるが、`global_string_pool_len` までは promote されない。`global_string_pool_len` の昇格は `ReturnFrame::TopLevel` の pop 時にしか行われず、`END` は IMMEDIATE な primitive として `VM::run` を経由せず直接呼ばれるためである。結果として、word を呼び出した時点ではリテラル文字列がフレームローカル扱いとなり、`SET &G, "..."` のような書き込みは `StringFrameEscape` で拒否される。

これは配列に対する `ArrayFrameEscape` と対称の制約であり、現状の設計の意図的な帰結である。word 内から長寿命の文字列をグローバル変数へ書き込みたい場合は、次のいずれかを用いる。

- 文字列リテラルをトップレベルでグローバル変数に代入してから、word 内では既存のグローバルを参照する
- word 内ではローカル `VAR` に代入し、`RETURN` で呼び出し元に返して呼び出し元側で `SET` する（`RETURN_VAL` の所有権移転によりフレーム境界を越えられる）

この制約の回帰確認テストは `src/vm.rs` の `test_str_literal_inside_word_assigned_to_global_var_is_currently_rejected` ほかにある。将来この挙動を緩める場合は、IMMEDIATE primitive の `END` 経路でも文字列プールが promote されるよう手当てした上で、上記テストを更新する。
...
新規の文字列リテラルおよび動的文字列はすべてランタイム文字列プール `VM::strings` に格納し、`Cell::Str(idx)` で参照する。length-prefixed バイト列で保持していた旧 `VM::string_pool` は、互換のため読み取り側にのみ残っている（issue #539 Phase 3 で削除予定）。
