...
#### `Str` の配列要素格納（Phase 5A）

> Issue #559 / #560 / #566 / #567 / #562（Phase 5A）に基づく設計方針

Phase 5A では、配列要素に一部の `Str` を格納できるようにした。`Cell::Str` の許可・拒否はライフタイム（`PoolRefLifetime`）の組み合わせで決まる。
...
Global or CallerOwned       CallerOwned       deny
```

具体的なルール：

- **global string**（compile-time literal を含む）は、任意の配列に格納できる。コンパイル済みワード内の文字列リテラルは session-lived / global として扱われるため、frame-local 配列への格納も安全である。
- **frame-local runtime string**（`STR_CONCAT`・`STR_SLICE` などが実行時に生成した文字列）は、いかなる配列にも格納できない。現在のコールフレームが終了すると解放されるため、配列に入れると dangling reference になる。
- **caller-owned string**（呼び出し元フレームに属する文字列パラメータなど）は、**FrameLocal 配列にのみ**格納できる。FrameLocal 配列は現在フレームに束縛されており、caller-owned string の所有フレームより先に解放されるため安全である。CallerOwned 配列や Global 配列への格納は拒否する（`PoolRefLifetime` は「どの外側フレームに属するか」を追跡しないため、二つの CallerOwned 値が異なるフレームに属する可能性を排除できない）。
- **nested Array**（`Cell::Array`）は引き続き無条件で拒否する。

拒否された場合のエラー：

- frame-local / unsafe lifetime の `Str` を配列に格納しようとすると `TbxError::StringFrameEscape`
- `Cell::Array` を配列要素に格納しようとすると `TbxError::InvalidArrayElement { got: "Array" }`
- `TO_ARRAY(...)` は `Cell::Str` を一律拒否する（`TbxError::InvalidArrayElement { got: "Str" }`）。`TO_ARRAY` はライフタイム文脈を持たないため、安全側に倒して全 `Str` を禁止している。

#### Phase 5B への残作業

以下は Phase 5A では扱わず、Phase 5B 以降に持ち越す。

- **frame-local runtime string の clone / promote**: `STR_CONCAT` などが生成した frame-local string を配列に入れるには、string pool を昇格させる仕組みが必要。
- **CallerOwned → Global/CallerOwned 配列への格納**: owner generation / frame depth のような、`CallerOwned` より細かい寿命追跡か、clone / promote による string stabilization を導入すれば安全化できる可能性がある。
- **配列返却時の要素内 string 安全化**: array return 時に要素内の string を昇格させる仕組み。
- **`TO_ARRAY(...)` への `Str` 対応**: ライフタイム検査を追加すれば global string のみ許可できるが、Phase 5B の課題とする。
...
> Issue #539 / #542（Phase 2）に基づく現状
>
> 新規に生成される文字列値は、文字列リテラルを含めて `Cell::Str` として `VM::strings` に格納する。静的・動的の区別はランタイム表現ではなく寿命管理の概念として整理を続ける。
...
Phase 2 以降、文字列リテラルは `Cell::Str` として `VM::strings` に格納される。ただし、文字列リテラルは実行時に生成される frame-local string ではなく、コンパイル済みコードから参照される compile-time literal である。

そのため、`DEF ... END` 本体中に現れる文字列リテラルも session-lived / global string として扱う。具体的には、`compile_expr` がリテラルを `VM::strings` に push した直後に `global_string_pool_len` を進めることで、word 呼び出し時にもグローバル変数へ格納できる。
...
このとき `"inside"` は `StringFrameEscape` の対象にはならない。一方、`STR_CONCAT` や `STR_SLICE` など word 呼び出し中に実行時生成された文字列は frame-local であり、`RETURN` による所有権移転を経由しない限りグローバル変数へ逃がせない。これは配列に対する `ArrayFrameEscape` と対称の制約として残る。

回帰テストは `src/vm.rs` の `test_str_literal_inside_word_can_be_assigned_to_global_var` / `test_str_literal_inside_word_lives_in_global_region` / `test_str_frame_escape_via_store_to_dict_is_error` を参照。
...
文字列リテラルの保存方式は、辞書（コードセグメント）の管理と独立に扱う。文字列の実体は辞書とは別のプールに格納し、辞書には「プール内のハンドル」だけを書き込む。これにより文字列の生成・破棄を辞書のライフサイクルから切り離せる。

新規の文字列リテラルおよび動的文字列はすべてランタイム文字列プール `VM::strings` に格納し、`Cell::Str(idx)` で参照する。
