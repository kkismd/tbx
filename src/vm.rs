        ...
        loop {
            ...
            match entry_kind {
                ...
                EntryKind::Array { .. } => {
                    ...
                    // inline operands, which is always a bug.
                    return Err(TbxError::InvalidExpression {
                        reason: "array must be accessed via OFFSET, not dispatched directly",
                    });
                }
            }
        }
...
mod tests {
    ...
    fn test_array_direct_dispatch_returns_error() {
        ...
        let err = vm.run(prog_start).unwrap_err();
        assert!(
            matches!(err, crate::error::TbxError::InvalidExpression { .. }),
            "direct Array dispatch should return InvalidExpression, got: {err:?}"
        );
    }
}
