...
impl VM {
    ...
    /// Read a jump target address from the dictionary at the given offset.
    ///
    /// Jump targets are stored as `Cell::DictAddr`.
    ///
    /// # Errors
    ///
    /// - `Err(TbxError::IndexOutOfBounds)` if `offset` is beyond the dictionary end.
    /// - `Err(TbxError::TypeError)` if the cell at `offset` is not a `Cell::DictAddr`.
    fn read_jump_target(&self, offset: usize) -> Result<usize, TbxError> {
        let cell = self.dict_read(offset)?;
        cell.as_dict_addr().ok_or_else(|| TbxError::TypeError {
        ...
        loop {
            ...
            match entry_kind {
                ...
                EntryKind::Offset => {
                    ...
                    let base_raw = base_cell.as_int().ok_or_else(|| TbxError::TypeError {
                    ...
                    let size_raw = size_cell.as_int().ok_or_else(|| TbxError::TypeError {
                ...
                EntryKind::Array { .. } => {
                    // Arrays must be accessed via the OFFSET instruction generated by the
                    // expression compiler.  Reaching this arm directly means an Xt for an
                    // array was placed in the code stream without the surrounding OFFSET
                    // inline operands, which is always a runtime error.
                    return Err(TbxError::TypeError {
                        expected: "OFFSET instruction before Array Xt",
                        got: "Array (direct dispatch)",
                    });
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_run_goto_wrong_type_target_errors() {
        // GOTO with a non-DictAddr operand (Cell::Int) must return TypeError.
        let mut vm = VM::new();
        crate::primitives::register_all(&mut vm);
        ...
        vm.dict_write(Cell::Xt(goto_xt)).unwrap(); // [0]
        vm.dict_write(Cell::Int(1)).unwrap(); // [1] Int instead of DictAddr
    ...
    #[test]
    fn test_run_goto_non_dict_addr_target_errors() {
        // GOTO with a Cell::Xt operand must return TypeError with got = "Xt".
        let mut vm = VM::new();
        crate::primitives::register_all(&mut vm);
    ...
    #[test]
    fn test_run_bif_wrong_type_target_errors() {
        // BIF with a non-DictAddr target (Cell::Int) must return TypeError.
        let mut vm = VM::new();
        crate::primitives::register_all(&mut vm);
        ...
        vm.dict_write(Cell::Xt(bif_xt)).unwrap(); // [0]
        vm.dict_write(Cell::Int(5)).unwrap(); // [1] Int instead of DictAddr
    ...
    #[test]
    fn test_run_bit_wrong_type_target_errors() {
        // BIT with a non-DictAddr target (Cell::Int) must return TypeError.
        let mut vm = VM::new();
        crate::primitives::register_all(&mut vm);
        ...
        vm.dict_write(Cell::Xt(bit_xt)).unwrap(); // [0]
        vm.dict_write(Cell::Int(3)).unwrap(); // [1] Int instead of DictAddr
    ...
    #[test]
    fn test_run_bif_wrong_type_target_errors_on_fallthrough() {
        // BIF with a non-DictAddr target must return TypeError even on fall-through.
    ...
    #[test]
    fn test_run_bit_wrong_type_target_errors_on_fallthrough() {
        // BIT with a non-DictAddr target must return TypeError even on fall-through.
        // read_jump_target is always evaluated regardless of the condition.
        let mut vm = VM::new();
        crate::primitives::register_all(&mut vm);
        ...
        vm.dict_write(Cell::Xt(bit_xt)).unwrap(); // [0]
        vm.dict_write(Cell::Int(2)).unwrap(); // [1] Int instead of DictAddr
    ...
    // ------------------------------------------------------------------
    // OFFSET instruction runtime tests
    // ------------------------------------------------------------------

    /// Build a minimal VM with all system primitives registered, then write a
    /// small program directly into the dictionary and run it.
    fn make_runtime_vm() -> VM {
        crate::init_vm()
    }

    /// Helper: write an OFFSET instruction with inline base and size operands.
    fn write_offset_instr(vm: &mut VM, base: usize, size: usize) -> usize {
        let offset_xt = vm.lookup("OFFSET").expect("OFFSET must be registered");
        let start = vm.dp;
        vm.dict_write(Cell::Xt(offset_xt)).unwrap();
        vm.dict_write(Cell::Int(base as i64)).unwrap();
        vm.dict_write(Cell::Int(size as i64)).unwrap();
        start
    }

    /// Helper: write a HALT instruction and return its position.
    fn write_halt(vm: &mut VM) -> usize {
        let halt_xt = vm.lookup("HALT").expect("HALT must be registered");
        let pos = vm.dp;
        vm.dict_write(Cell::Xt(halt_xt)).unwrap();
        pos
    }

    #[test]
    fn test_offset_valid_index_pushes_dict_addr() {
        // Program: OFFSET(base=10, size=5) with idx=2 on the stack.
        // Expected result: DictAddr(12) on the data stack.
        let mut vm = make_runtime_vm();
        let base = 10usize;
        let size = 5usize;

        let prog_start = write_offset_instr(&mut vm, base, size);
        write_halt(&mut vm);

        // Pre-push the index.
        vm.push(Cell::Int(2)).unwrap();

        let result = vm.run(prog_start);
        assert!(
            matches!(result, Err(crate::error::TbxError::Halted)),
            "run should halt normally, got: {result:?}"
        );
        let top = vm.pop().unwrap();
        assert_eq!(
            top,
            Cell::DictAddr(base + 2),
            "OFFSET should push DictAddr(base+idx)"
        );
    }

    #[test]
    fn test_offset_index_zero_is_valid() {
        // Boundary: idx=0, size=1 — valid, should push DictAddr(base).
        let mut vm = make_runtime_vm();
        let base = 5usize;
        let size = 1usize;
        ...
        vm.push(Cell::Int(0)).unwrap();
        let result = vm.run(prog_start);
        assert!(matches!(result, Err(crate::error::TbxError::Halted)));
        assert_eq!(vm.pop().unwrap(), Cell::DictAddr(base));
    }

    #[test]
    fn test_offset_negative_index_returns_error() {
        // idx=-1 should produce ArrayIndexOutOfBounds.
        let mut vm = make_runtime_vm();
        let base = 10usize;
        let size = 3usize;
        ...
        vm.push(Cell::Int(-1)).unwrap();
        let err = vm.run(prog_start).unwrap_err();
        assert!(
            matches!(
                err,
                crate::error::TbxError::ArrayIndexOutOfBounds { index: -1, size: 3 }
            ),
            "expected ArrayIndexOutOfBounds for idx=-1, got: {err:?}"
        );
    }

    #[test]
    fn test_offset_index_equal_to_size_returns_error() {
        // idx==size (upper bound exclusive) should produce ArrayIndexOutOfBounds.
        let mut vm = make_runtime_vm();
        let base = 10usize;
        let size = 3usize;
        ...
        vm.push(Cell::Int(size as i64)).unwrap();
        let err = vm.run(prog_start).unwrap_err();
        assert!(
            matches!(
                err,
                crate::error::TbxError::ArrayIndexOutOfBounds { index: 3, size: 3 }
            ),
            "expected ArrayIndexOutOfBounds for idx=size, got: {err:?}"
        );
    }

    #[test]
    fn test_offset_index_exceeds_size_returns_error() {
        // idx > size should also produce ArrayIndexOutOfBounds.
        let mut vm = make_runtime_vm();
        let base = 10usize;
        let size = 3usize;
        ...
        assert!(
            matches!(
                err,
                crate::error::TbxError::ArrayIndexOutOfBounds {
                    index: 100,
                    size: 3
                }
            ),
            "expected ArrayIndexOutOfBounds for idx=100, got: {err:?}"
        );
    }

    #[test]
    fn test_array_direct_dispatch_returns_error() {
        // Directly dispatching an Array Xt (without OFFSET) must return an error.
        let mut vm = make_runtime_vm();

        // Allocate 3 cells and register an array.
        ...
        // Write the array's Xt directly into the code stream (invalid).
        let arr_xt = vm.lookup("ARR").expect("ARR must be registered");
        let prog_start = vm.dp;
        vm.dict_write(Cell::Xt(arr_xt)).unwrap();
