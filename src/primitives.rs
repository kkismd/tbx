...
enum PoolRef {
    Array(usize),
    // TODO: remove String variant when VM::strings pool cleanup is complete (follow-up issue).
    #[allow(dead_code)]
    String(usize),
}
...
fn pool_ref_from_cell(cell: &Cell) -> Option<PoolRef> {
    match cell {
        ...
        // Cell::Str is now Rc<str>-backed and does not have a pool index.
        // String lifetime is managed by Rc reference counting; no PoolRef needed.
        Cell::Str(_) => None,
        _ => None,
    }
}
...
/// `Cell::Str` is now `Rc<str>`-backed and is accepted; lifetime is managed by
/// reference counting.
...
/// `array_pool_idx`.
///
/// This function is retained from Phase 5A for reference and future cleanup.
/// With `Cell::Str` now being `Rc<str>`-backed, the lifetime checks it performs
/// are no longer used in the active code path.
///
/// TODO: remove when VM::strings pool cleanup is complete (follow-up issue).
#[allow(dead_code)]
...
///
/// * `Cell::Array` is always rejected (nested arrays are not supported).
/// * `Cell::Str` is now `Rc<str>`-backed; its lifetime is managed by reference
///   counting, so it is always safe to store in any array.
/// * All other types are accepted unconditionally.
fn check_array_element_write(
    ...
    match value {
        ...
        Cell::Array(_) => Err(TbxError::InvalidArrayElement { got: "Array" }),
        // Cell::Str is Rc<str>-backed; accepted in any array.
        // All other scalar types are also accepted.
        _ => Ok(()),
    }
}
...
pub fn str_prim(vm: &mut VM) -> Result<(), TbxError> {
    ...
    let s: std::rc::Rc<str> = match &cell {
        // For Str, return the same Rc (identity-like conversion).
        Cell::Str(s) => s.clone(),
...
/// Helper: resolve a `Cell::Str` to a `String`.
...
mod tests {
    ...
    fn test_eq_str_different_handles_same_content_is_true() {
        // Two distinct Cell::Str handles with identical content compare equal.
        let mut vm = VM::new();
        vm.push(Cell::string("hello")).unwrap();
        vm.push(Cell::string("hello")).unwrap();
        eq_prim(&mut vm).unwrap();
    ...
    fn test_str_prim_from_cell_str() {
        ...
        str_prim(&mut vm).unwrap();
        // STR on a Str returns the same content (Rc clone).
    ...
    #[test]
    fn test_str_eq_same_content_via_different_handles_is_equal() {
        let mut vm = VM::new();
    ...
    // Note: Cell::Str is now Rc<str>-backed; it can always be stored in a dict slot
    // without a StringFrameEscape error.  The old pool-index-based frame checks are
    // no longer applicable to Str cells.

    #[test]
    fn test_str_can_be_stored_in_dict_addr() {
        // Cell::Str(Rc<str>) may always be stored into a DictAddr without error.
        let mut vm = VM::new();
        ...
        vm.push(Cell::DictAddr(0)).unwrap();
        store_prim(&mut vm).unwrap();
        assert_eq!(vm.dictionary[0], Cell::string("hello"));
    }

    #[test]
    fn test_str_stored_in_dict_addr_roundtrip() {
        // Store a Str then fetch it back; content must be preserved.
        let mut vm = VM::new();
        vm.dictionary.push(Cell::None);
        vm.dp = 1;
        vm.push(Cell::string("world")).unwrap();
    ...
    fn test_pool_ref_from_cell_array_only() {
        // Cell::Array produces a PoolRef; Cell::Str is now Rc<str>-backed and returns None.
    ...
    fn test_cs_open_tag_pushes_tag_to_compile_stack() {
        // CS_OPEN_TAG must pop a Cell::Str and push Tag to compile_stack.
        let mut vm = make_compiling_vm("TESTWORD");
    ...
    // --- array element write: Cell::Str (Rc<str>) ---
    // Cell::Str is now Rc<str>-backed; its lifetime is managed by reference counting,
    // so it can be stored in any array without a StringFrameEscape error.

    #[test]
    fn test_set_str_to_array_element_is_allowed() {
        // Cell::Str(Rc<str>) may always be stored into an array element.
        let mut vm = VM::new();
