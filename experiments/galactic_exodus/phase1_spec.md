...
- 成功移動または未知断層失敗後にremaining fuelが0となり、Hへ到着していない場合は`LOST_FUEL`とする。

### 5.3 通常敗北に含めない状態（WIN-003）
...
known stateだけから推測した行動不能を通常敗北条件にしない。actual mapを参照した「真の行動不能」判定も通常ゲームUIでは行わない。
...
- cells: `{position, symbol}`の配列を`y,x`昇順で保持
