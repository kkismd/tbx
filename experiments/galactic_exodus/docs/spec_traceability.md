...
|---|---|---|---|---|---|
| `LEGACY_CANONICAL_SPEC` | `experiments/galactic_exodus/phase1_spec.md` | `experiments/galactic_exodus/docs/archive/phase1_spec.md` | `archived` | `experiments/galactic_exodus/docs/specs/lrs_gameplay.md` | #1318 / #1354 |
...
|---|---|---|---|---|---|---|---|---|
| `experiments/galactic_exodus/phase1_spec.md` | `## 1` | Phase 1 authority / inputs / decision register | `docs/specs/lrs_gameplay.md` | `lrs_gameplay.md` が `CURRENT_SOURCE` として authority、schema version、decision register、fixture validator を current path で固定した。 | `MIGRATED` | なし | 維持のみ。 | #1323 / #1354 |
| `experiments/galactic_exodus/phase1_spec.md` | `## 2-3` | board / known state / movement contract | `docs/specs/lrs_gameplay.md` | fixed 8x8 board、`known_cells` / `visited_cells` / `known_routes`、unknown / known `RIFT`、movement resolution order、LRS sector間edge topology を current gameplay contract として移植した。 | `MIGRATED` | なし | 維持のみ。 | #1323 / #1354 |
| `experiments/galactic_exodus/phase1_spec.md` | `## 4-6` | fuel / supply / win-lose / seed / fixture injection | `docs/specs/lrs_gameplay.md` | fuel、`BASE` / `RESOURCE`、`WON` / `LOST_FUEL` / abort、seed reroll、fixture injection を current contract として移植した。 | `MIGRATED` | なし | 維持のみ。 | #1323 / #1354 |
| `experiments/galactic_exodus/phase1_spec.md` | `## 7-10` | UI contract / GameLog / comparison / reference fixture | `docs/specs/lrs_gameplay.md` | LRS UI meaning contract、`GameLog` schema v3、`TurnEvent` / `final_summary` field、Python/TBX 比較境界、reference fixture schema v1 を current docs へ移植した。 | `MIGRATED` | なし | 維持のみ。 | #1323 / #1354 |
| `experiments/galactic_exodus/phase1_spec.md` | `## 11-14` | historical notes / implementation order | `docs/specs/lrs_gameplay.md` appendix または legacy のまま | `lrs_gameplay.md` は `historical appendix` を設け、履歴情報を current gameplay 本文から分離した。詳細な履歴本文は legacy 文書を参照する。 | `OBSOLETE` | current source にそのまま移すと歴史メモと仕様を混ぜる。 | 維持のみ。 | #1323 / #1354 |
...
| `experiments/galactic_exodus/srs/phase2_srs_elements.md` | `## 2` | 11x11 candidate board size | legacy のまま | current docs は 11x11 candidate を current baseline に含めていない。 | `OBSOLETE` | legacy の候補値を current baseline 仕様へ混ぜると authority がぶれる。 | `#1321` で legacy note を追加し、`11x11` candidate が historical であることと current `9x9` baseline を明記した。 | #1321 |
...
| `experiments/galactic_exodus/srs/phase2_srs_elements.md` | `## 10` | old `WARP_POINT` placement | `docs/specs/srs_warp.md`, `docs/specs/srs_map_generation.md` | current docs は `warp_flags` 表現へ移行済み。 | `OBSOLETE` | object/feature としての `WARP_POINT` 前提が current authority と合わない。 | `#1321` で legacy note を追加し、`warp_flags` への置換と historical scope を明記した。 | #1321 |
| `experiments/galactic_exodus/srs/phase2_srs_elements.md` | `## 11-12` | SectorType x Terrain / Terrain x Object matrices | `docs/specs/srs_map_generation.md`, `docs/specs/srs_objects.md`, `docs/specs/srs_warp.md` | object placement と current prototype generator の範囲は docs 化済み。 | `PARTIAL` | legacy matrix は full terrain-count profile と 11x11 range を含むが、current `srs_map_generation.md` は terrain-count profile を deferred 扱いにしている。 | `#1321` で legacy note を追加し、current implementation 範囲と deferred / historical 範囲の境界を明示した。 | #1321 |
...
| `experiments/galactic_exodus/srs/phase2_srs_spec.md` | `## 7` | object placement and lifecycle | `docs/specs/srs_movement.md`, `docs/specs/srs_objects.md`, `docs/specs/srs_map_generation.md` | current docs に object lifecycle が分割済み。 | `CONFLICTING` | legacy は `SALVAGE` の即時回復 choice に `RECOVER_DURABILITY` を含む一方、current `srs_objects.md` は `RECOVER_ENERGY` / `RECOVER_PHOTON_TORPEDO_AMMO` / `STORE_ONLY` のみに固定している。 | `#1321` で legacy note を追加し、`SALVAGE` choice set の競合と current authority を明記した。 | #1321 |
...
| `experiments/galactic_exodus/srs/phase2_srs_spec.md` | `## 11` | combat initial model | `docs/specs/srs_combat.md` | `srs_combat.md` が combat state / command / reaction / event contract の current authority として追加された。legacy path は参照根拠として残すが、authority は current spec へ移った。 | `MIGRATED` | なし | 維持のみ。 | #1319 |
| `experiments/galactic_exodus/srs/phase2_srs_spec.md` | `## 12` | encounter initial model | `docs/specs/srs_encounter.md` | `srs_encounter.md` が encounter roll / suppression / spawn / `drop_salvage` / payload の current authority として追加された。legacy path は根拠として残るが、authority は current spec へ移った。 | `MIGRATED` | なし | 維持のみ。 | #1320 |
...
| `experiments/galactic_exodus/srs/phase2_srs_spec.md` | `## 16` | GameLog and comparison contract | `docs/specs/srs_movement.md`, `docs/specs/srs_combat.md`, `docs/specs/srs_encounter.md`, `docs/specs/display.md` | movement / combat / encounter / display payload と renderer / formatter 境界を current docs へ分割移植済み。 | `MIGRATED` | なし | 維持のみ。 | - |
| `experiments/galactic_exodus/srs/phase2_srs_spec.md` | `## 17` | display-input contract for #1076 | `docs/specs/display.md` | `display.md` が LRS / SRS map、HUD、event formatter、manual-eval 境界の current authority として追加された。 | `MIGRATED` | なし | 維持のみ。 | - |
| `experiments/galactic_exodus/srs/phase2_srs_spec.md` | `## 18` | deferred follow-up map | `docs/specs/README.md` と各 current spec header | current docs は source issue / follow-up を各 file header で持つ。 | `OBSOLETE` | follow-up map を旧 monolithic spec で維持する必要は薄い。 | `#1321` で legacy note を追加し、current spec index と各 header を authority として参照させた。 | #1321 |
...
| `experiments/galactic_exodus/srs/phase2_srs_movement.md` | `## 6-7` | `VECTOR_COMMAND` / `DIRECTIONAL_THRUST` comparison modes | legacy のまま | current `srs_movement.md` は baseline 不採用の比較候補として短く触れるのみ。 | `OBSOLETE` | 現行 Phase 2 baseline の authority ではない。 | `#1321` で legacy note を追加し、historical comparison candidate であることを明記した。 | #1321 |
...
| `experiments/galactic_exodus/srs/phase2_initial_model.md` | `## 5` | 11x11 candidate counts | legacy のまま | current docs は 11x11 candidate を current baseline に含めていない。 | `OBSOLETE` | current generator contract と同じ status では扱えない。 | `#1321` で legacy note を追加し、`11x11` candidate counts を historical として切り離した。 | #1321 |
| `experiments/galactic_exodus/srs/phase2_initial_model.md` | `## 6` | staged rollout memo | legacy のまま | 実装段階メモは current gameplay spec 本文ではない。 | `OBSOLETE` | gameplay contract と rollout memo は処分先が異なる。 | `#1321` で legacy note を追加し、rollout memo が gameplay authority ではないことを明記した。 | #1321 |
| `experiments/galactic_exodus/srs/phase2_initial_model.md` | `## 7` | evaluation assumptions / comparison IDs | `docs/specs/README.md`, `docs/specs/srs_movement.md` | baseline 前提の一部は current docs にある。 | `PARTIAL` | evaluation-specific な比較IDと質問票までは current docs へ全面移植していない。 | `#1321` で legacy note を追加し、current contract と historical evaluation memo の境界を明示した。 | #1321 |
| `experiments/galactic_exodus/srs/phase2_srs_generation.md` | `## 1-4` | generation scope / terrain-count profile / placement constraints | `docs/specs/srs_map_generation.md` | current docs は generator の現行範囲と deferred 範囲を明記している。 | `PARTIAL` | legacy は full terrain-count range / seed/report schema を正本としているが、current docs は terrain-count profile を deferred にしている。 | `#1321` で legacy note を追加し、current implementation と deferred / historical 範囲の境界を明示した。 | #1321 |
...
| `experiments/galactic_exodus/srs/phase2_srs_generation.md` | `## 6-9` | generation order / cluster generation / reachability / seed retry report | `docs/specs/srs_map_generation.md` | 現行 generator 手順と board-edge decision は docs 化済み。 | `PARTIAL` | legacy の retry/report schema を current docs はまだ全面移植していない。 | `#1321` で legacy note を追加し、current generator contract と retry/report の deferred 範囲を補足した。 | #1321 |
