...
| `experiments/galactic_exodus/srs/phase2_srs_elements.md` | `## 2-4` | board size candidate / terrain-object taxonomy | `docs/specs/srs_movement.md`, `docs/specs/srs_map_generation.md`, `docs/specs/srs_objects.md` | terrain/object taxonomy は split migration 済み。 | `PARTIAL` | legacy は `9x9/11x11` 候補と `WARP_POINT` 用語を残し、current docs は 9x9 baseline + `warp_flags` を正本化している。 | superseded note を追加し、current destination を明記する。 | #1323 |
...
| `experiments/galactic_exodus/srs/phase2_srs_elements.md` | `## 8-10` | gravity field rule / STOP_BEFORE / old `WARP_POINT` placement | `docs/specs/srs_movement.md`, `docs/specs/srs_warp.md`, `docs/specs/srs_map_generation.md` | STOP_BEFORE と `warp_flags` 表現は current docs にある。 | `PARTIAL` | gravity field directional multiplier は current docs と一致せず、`WARP_POINT` placement section は object/feature model 前提で古い。 | STOP_BEFORE は移植済みとして残し、重力値と `WARP_POINT` section には superseded note を入れる。 | #1323 |
...
| `experiments/galactic_exodus/srs/phase2_initial_model.md` | `## 5-7` | map size / celestial counts / staged rollout / evaluation assumptions | `docs/specs/srs_map_generation.md`, `docs/specs/srs_movement.md`, `docs/specs/README.md` | 天体数や baseline 候補の一部は current docs にある。 | `PARTIAL` | `11x11` range と staged implementation memo は current baseline 仕様ではない。 | gameplay contract と historical evaluation memo を切り分ける。 | #1323 |
