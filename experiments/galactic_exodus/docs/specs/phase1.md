# Galactic Exodus Phase 1仕様

Source issue: #1323
Parent issue: #1314
Depends on: #1313, #1314, #1317, #1321, #1322
Related: #1040, #1047, #1058, #1076, #1259, #1307, #1318
Base branch: `integration/882-galactic-exodus`

この文書は、Galactic Exodus Phase 1 LRS prototype に対する唯一の `CURRENT_SOURCE` である。

- 対象は `experiments/galactic_exodus/phase1_spec.md` の `## 1-10` に含まれていた current gameplay / UI / logging / fixture contract である
...
この文書は、Phase 1 LRS gameplay contract、Phase 1 UI meaning contract、`GameLog` schema v3、reference fixture schema v1 を固定する `CURRENT_SOURCE` である。

authority 優先順位:

1. merged decision issue と `experiments/galactic_exodus/docs/specs/` 配下の current docs
2. current implementation / tests / fixtures
3. `experiments/galactic_exodus/phase1_spec.md` の `## 1-10`
4. evaluation reports
5. `experiments/galactic_exodus/phase1_spec.md` の `## 11-14`
...
- Phase 2 SRS movement: [`srs_movement.md`](srs_movement.md)
- Phase 2 map / warp / object / combat / encounter: [`srs_map_generation.md`](srs_map_generation.md), [`srs_warp.md`](srs_warp.md), [`srs_objects.md`](srs_objects.md), [`srs_combat.md`](srs_combat.md), [`srs_encounter.md`](srs_encounter.md)
...
- Phase 1 数値の再調整
- Phase 2 仕様の再記述
- `experiments/galactic_exodus/phase1_spec.md` の削除、移動、変更
...
## 15. Phase 1 UI 入力・表示契約

この章は Phase 1 normal UI に必要な意味契約のみを固定する。current renderer の具体 layout や wording は [`display.md`](display.md) を優先する。

Phase 1 normal UI が区別すべき意味:
...
- Phase 1 本文は「何を区別できる必要があるか」を固定する
...
evaluation documents は Phase 1 の `CURRENT_SOURCE` ではない。仕様に競合がある場合はこの文書と current implementation / tests を優先する。

## 22. historical appendix

`experiments/galactic_exodus/phase1_spec.md` の `## 11-14` は current gameplay contract ではなく、次の履歴情報として扱う。
...
履歴の詳細は `experiments/galactic_exodus/phase1_spec.md` と関連 issue を参照する。

## 23. deferred / non-scope

deferred:

- Braille、色、端末幅別 layout、screen-reader 向け代替表示などの詳細表示設計は `display.md` と Phase 2 系 issue へ委譲する
