...
- バグ・ロジックエラー
- `blueprint.md` / `blueprint-language.md` / `blueprint-compiler.md` の設計方針との乖離
- Rustの安全性・慣用的でない記述
- テストの欠落・不十分なカバレッジ
...
ローカルにPRのブランチがチェックアウト済みの場合は、コードの読み取りにローカルファイルを直接使用する（GitHub APIを経由しない）。差分・ファイル一覧などのメタ情報は引き続きMCPツールで取得する。
...
`blueprint.md`（およびそこから参照される `blueprint-language.md`・`blueprint-compiler.md`）を読み込み、レビュー対象のコードに関連する設計方針を特定する。
...
問題が見つからない場合はApproveコメントをPRに投稿し、Issue登録は行わない。
