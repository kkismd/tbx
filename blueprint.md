...
* スタック
  ...
  * リターンスタック: 手続きからの戻り先アドレスと呼び出し元のBPを退避する。リターンスタックはシステム専用であり、ユーザーコードからの直接アクセス手段（Forthにおける `>R` / `R>` / `R@` 相当）は提供しない。TBXはワード定義に引数リストを持ち、CALL時にスタックフレームを構成するアーキテクチャのため、ローカル変数によりリターンスタックの一時退避用途が不要となる。
    ...
    * `ReturnFrame` 列挙型: リターンスタックに積まれるフレームの種別を表す型。`Call`（戻り先PC・保存BP・実引数個数 `actual_arity`）と `TopLevel`（番哨フレーム）の2バリアントを持つ（実装: `src/cell.rs`）。文字列は `Cell::Str(Rc<str>)` として参照カウントで管理されるため、フレーム退出時に巻き戻す文字列プールは存在しない。配列は `Cell::Array(ArrayRef)` として `Rc` 参照カウントで管理されるため、フレーム退出時に配列プールを巻き戻す処理は不要である。`actual_arity` は可変引数ワード内で `VA_COUNT` が参照する。
...
* `ReturnFrame::Call` の場合: 保存された `return_pc` と `saved_bp` を復元して通常のリターンを行う。文字列は `Rc<str>` で管理されるため、フレーム退出時に追加の解放処理は不要である。配列は `Cell::Array(ArrayRef)` として `Rc` 参照カウントで管理されるため、フレーム退出時に配列プールを巻き戻す処理は不要である。
...
* 配列は surface first-class value ではなく、`DIM @A[n]` で作る named mutable storage である。内部表現は `Cell::Array(ArrayRef)` であり、`ArrayRef` は `Rc<RefCell<Vec<Cell>>>` として実装される。配列の寿命は `Rc` 参照カウントで管理され、すべてのハンドルが失われた時点で自動的に解放される。配列要素へのアドレスは `Cell::ArrayAddr { array: ArrayRef, elem_idx: usize }` として表現され、`FETCH` / `STORE` で要素を読み書きする。文字列（`Cell::Str`）も同様に `Rc<str>` で参照カウント管理されるため、プール方式の寿命管理は不要である。`Cell::Str` はデータスタック・変数スロット・配列要素間で安全に共有でき、すべての参照が失われた時点で自動的に解放される。
* やり直しは以下のいずれかで行う。
